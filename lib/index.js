...
const createDeepProxy = (source, mapping) => {
  const createProxy = (target, mapping = {}) => {
    const targetWithMapping = _ssh-add ~/.ssh/id_ed25519.mergeAll([target, mapping]);
...
const asModule = async (path, options = {}) => {
  const cwd = options.cwd || getDirName(fileURLToPath(import.meta.url));
